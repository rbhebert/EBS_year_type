...
```stata
do build_ebs_year_type_wide.do
```
...
- The underlying EBS workbook is national, not state-specific.
- The resulting dataset is therefore national by `year x type`.
- The `.do` file is designed to run from the repository root and uses only the raw workbook stored in `raw_data/`.
